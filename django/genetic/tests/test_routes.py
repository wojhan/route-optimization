...
class RouteTestCase(unittest.TestCase):
    ...
    def __prepare_route(self, route_parts, max_distance = 10000):
        ...
        route.available_vertices = set(list(TestData.companies))
...
class AddingStopsTestCase(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        for i, vertex in enumerate(cls.depots + cls.companies + cls.hotels):
            ...
            for j, another_vertex in enumerate(cls.depots + cls.companies + cls.hotels):
                distances[vertex.uuid][another_vertex.uuid] = mpu.haversine_distance(vertex.get_coords(), another_vertex.get_coords())
    ...
    def test_add_on_first_in_start_route_part(self):
        ...
        route_part = RoutePart([self.depots[0], self.depots[0]])
        route.add_route_part(route_part)
        route.add_stop(route_part, 0, self.companies[0])
        expected_distance = self.__count_distance(self.depots[0], self.companies[0])
        ...
        self.assertEqual(route_part.route[0], self.companies[0])

    def test_add_on_first_the_same_as_first_in_start_route_part(self):
        route = Route(1, 10000, self.distances)
        route_part = RoutePart([self.depots[0], self.depots[0]])
        route.add_route_part(route_part)
        route.add_stop(route_part, 0, self.depots[0])
        expected_distance = 0
        expected_route_length = 3
        ...
        self.assertEqual(route_part.distance, expected_distance)
        self.assertEqual(route_part.length, expected_route_length)
        self.assertEqual(route_part.route[0], self.depots[0])

    def test_add_on_first_in_existing_route_part(self):
        route = Route(1, 10000, self.distances)
        route_part = RoutePart([self.depots[0], self.companies[0], self.depots[0]])
        route.add_route_part(route_part)
        route.add_stop(route_part, 0, self.companies[1])
        expected_distance = self.__count_distance(self.companies[1], self.depots[0]) + (self.__count_distance(self.depots[0], self.companies[0]) * 2)
        expected_route_length = 4

        self.assertEqual(route.distance, expected_distance)
        self.assertEqual(route_part.distance, expected_distance)
        self.assertEqual(route_part.length, expected_route_length)
        self.assertEqual(route_part.route[0], self.companies[1])
    ...
    def test_add_on_first_the_same_as_first_in_existing_route_part(self):
        ...
        route.add_route_part(route_part)
        route.add_stop(route_part, 0, self.depots[0])
        expected_distance = self.__count_distance(self.depots[0], self.companies[0]) * 2
        expected_route_length = 4
    ...
    def test_add_on_last_in_start_route_part(self):
        ...
        route_part = RoutePart([self.depots[0], self.depots[0]])
        route.add_route_part(route_part)
        route.add_stop(route_part, 2, self.companies[0])
        expected_distance = self.__count_distance(self.depots[0], self.companies[0])
        expected_route_length = 3
        ...
        self.assertEqual(route_part.distance, expected_distance)
        self.assertEqual(route_part.length, expected_route_length)
        self.assertEqual(route_part.route[-1], self.companies[0])

    def test_add_on_last_the_same_as_last_in_start_route_part(self):
        route = Route(1, 10000, self.distances)
        route_part = RoutePart([self.depots[0], self.depots[0]])
        route.add_route_part(route_part)
        route.add_stop(route_part, route_part.length, self.depots[0])
        expected_distance = 0
        expected_route_length = 3
    ...
    def test_add_on_last_in_existing_route_part(self):
        ...
        route_part = RoutePart([self.depots[0], self.companies[0], self.depots[0]])
        route.add_route_part(route_part)
        route.add_stop(route_part, route_part.length, self.companies[1])
        expected_distance = self.__count_distance(self.companies[1], self.depots[0]) + (self.__count_distance(self.depots[0], self.companies[0]) * 2)
        expected_route_length = 4
        ...
        self.assertEqual(route_part.distance, expected_distance)
        self.assertEqual(route_part.length, expected_route_length)
        self.assertEqual(route_part.route[-1], self.companies[1])
    ...
    def test_add_on_last_the_same_as_last_in_existing_route_part(self):
        ...
        self.assertEqual(route_part.distance, expected_distance)
        self.assertEqual(route_part.length, expected_route_length)
        self.assertEqual(route_part.route[-1], self.depots[0])

    def test_add_on_non_edge_position_in_start_route_part(self):
        route = Route(1, 10000, self.distances)
        route_part = RoutePart([self.depots[0], self.depots[0]])
        route.add_route_part(route_part)
        route.add_stop(route_part, 1, self.companies[0])
        expected_distance = self.__count_distance(self.depots[0], self.companies[0]) * 2
        expected_route_length = 3
    ...
    def test_add_on_non_edge_position_in_existing_route_part(self):
        ...
        route_part = RoutePart([self.depots[0], self.companies[0], self.depots[0]])
        route.add_route_part(route_part)
        route.add_stop(route_part, 1, self.companies[1])
        expected_distance = self.__count_distance(self.depots[0], self.companies[0]) + self.__count_distance(self.companies[0], self.companies[1]) + self.__count_distance(self.companies[1], self.depots[0])
        expected_route_length = 4
        ...
        self.assertEqual(route_part.length, expected_route_length)
        self.assertEqual(route_part.route[1], self.companies[1])
        self.assertEqual(route_part.route[2], self.companies[0])
...
class RemovingStopsTestCase(unittest.TestCase):
    ...
    def test_remove_first_vertex_in_existing_route_part(self):
        ...
        route.add_route_part(route_part)
        route.remove_stop(route_part, 0)
        expected_distance = self.__count_distance(self.companies[0], self.depots[0])
...
class ReplacingStopsTestCase(unittest.TestCase):
    ...
    def test_replace_first_element_in_route_part_gives_correct_distance(self):
        ...
        replacing_company = self.companies[0]
        expected_distance = self.__count_distance(self.companies[0], self.depots[0])
    ...
    def test_replace_not_edge_element_in_route_part_gives_correct_distance(self):
        route = Route(1, 10000, self.distances)
        route_part = RoutePart([self.depots[0], self.companies[1], self.depots[0]])
        route.add_route_part(route_part)

        replacing_company = self.companies[0]
        expected_distance = self.__count_distance(self.companies[0], self.depots[0]) * 2
...
class CrossoverTestCase(unittest.TestCase):
    ...
    def test_crossover_when_one_has_3_elements_and_another_is_bigger(self):
        ...
        route_parts = [
            ...
            RoutePart([self.depots[0], self.companies[1], self.companies[2], self.companies[3], self.depots[0]])
        ]

        routes[0].add_route_part(route_parts[0])
        routes[1].add_route_part(route_parts[1])

        expected_first_route = [self.depots[0], self.companies[0], self.companies[2], self.companies[3], self.depots[0]]
        expected_second_route = [self.depots[0], self.companies[1], self.depots[0]]
        expected_first_distance = self.__count_distance(self.depots[0], self.companies[0]) + self.__count_distance(self.companies[0], self.companies[2]) + self.__count_distance(self.companies[2], self.companies[3]) + self.__count_distance(self.companies[3], self.depots[0])
        expected_second_distance = self.__count_distance(self.depots[0], self.companies[1]) * 2
    ...
    def test_crossover_when_cross_index_is_on_first_half(self):
        ...
        expected_first_route = [self.depots[0], self.companies[0], self.companies[3], self.companies[4], self.depots[0]]
        expected_second_route = [self.depots[0], self.companies[2], self.companies[1], self.depots[0]]
        expected_first_distance = self.__count_distance(self.depots[0], self.companies[0]) + self.__count_distance(self.companies[0], self.companies[3]) + self.__count_distance(self.companies[3], self.companies[4]) + self.__count_distance(self.companies[4], self.depots[0])
        expected_second_distance = self.__count_distance(self.depots[0], self.companies[2]) + self.__count_distance(self.companies[2], self.companies[1]) + self.__count_distance(self.companies[1], self.depots[0])
    ...
    def test_crossover_when_cross_index_is_on_second_half(self):
        ...
        route_parts = [
            RoutePart([self.depots[0], self.companies[0], self.companies[1], self.companies[5], self.companies[6], self.depots[0]]),
            RoutePart([self.depots[0], self.companies[2], self.companies[3], self.companies[4], self.companies[7], self.depots[0]])
        ]

        routes[0].add_route_part(route_parts[0])
        routes[1].add_route_part(route_parts[1])

        expected_first_route = [self.depots[0], self.companies[0], self.companies[1], self.companies[5], self.companies[7], self.depots[0]]
        expected_second_route = [self.depots[0], self.companies[2], self.companies[3], self.companies[4], self.companies[6], self.depots[0]]
        expected_first_distance = self.__count_distance(self.depots[0], self.companies[0]) + self.__count_distance(self.companies[0], self.companies[1]) + self.__count_distance(self.companies[1], self.companies[5]) + self.__count_distance(self.companies[5], self.companies[7]) + self.__count_distance(self.companies[7], self.depots[0])
        expected_second_distance = self.__count_distance(self.depots[0], self.companies[2]) + self.__count_distance(self.companies[2], self.companies[3]) + self.__count_distance(self.companies[3], self.companies[4]) + self.__count_distance(self.companies[4], self.companies[6]) + self.__count_distance(self.companies[6], self.depots[0])
...
class ManagingStopsTestCase(TestCase):
    ...
    def test_add_and_remove_the_same_vertex_on_first_position_at_start_route_part_gives_correct_distance(self):
        ...
        expected_distance_after_add = self.__count_distance(self.depots[0], self.companies[0])
        ...
        self.assertEqual(route_part.route[0], self.companies[0])
        self.assertEqual(route_part.route[1], self.depots[0])
        self.assertEqual(route_part.route[2], self.depots[0])
    ...
    def test_add_and_remove_the_same_vertex_on_first_position_at_existing_route_part_gives_correct_distance(self):
        ...
        self.assertEqual(route.distance, self.__count_distance(self.companies[0], self.depots[0]) * 2)

        expected_route_length_after_add = 4
        expected_distance_after_add = self.__count_distance(self.companies[1], self.depots[0]) + self.__count_distance(self.depots[0], self.companies[0]) * 2
        expected_route_length_after_remove = 3
        expected_distance_after_remove = self.__count_distance(self.companies[0], self.depots[0]) * 2
        ...
        self.assertEqual(route_part.route[1], self.depots[0])
        self.assertEqual(route_part.route[2], self.companies[0])
        self.assertEqual(route_part.route[3], self.depots[0])
    ...
    def test_add_and_remove_n_vertices_on_route_part_gives_correct_distance(self):
        ...
        for i in range(n):
            ...
            random_company = self.companies[random.randint(0, len(self.companies) - 1)]

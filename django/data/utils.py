...
def generate_data_for_route(business_trip, data, crossover_probability=0.7, mutation_probability=0.4, elitism_rate=0.1,
    ...
    requisitions = [Requistion.objects.get(
        id=requistion_data['id']) for requistion_data in data['requistions']]

    depot_company = Company.objects.get(pk=4380)

    depots = dict(name=str(depot_company.pk), coords=(depot_company.latitude, depot_company.longitude))

    companies = []

    for requisition in requisitions:
        company = dict(name=str(requisition.company.pk),
                       coords=(requisition.company.latitude, requisition.company.longitude),
                       profit=requisition.estimated_profit)
        companies.append(company)

    hotels = [dict(name=str(hotel.pk), coords=(hotel.latitude, hotel.longitude)) for hotel in Hotel.objects.all()]
